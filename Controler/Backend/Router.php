...
class Router
{
            ...
            if ($this->ctrlAdminSecure()->autorizationEnter($_SESSION)) {
                ...
                try {
                    if (isset($_GET['action'])) {
                        if ($_GET['action'] == 'ticket') { // Affiche le formulaire d'ajout ou de modification d'un chapitre
                        ...
                        } elseif ($_GET['action'] == 'comment') { // Affiche les commentaires
                            $this->ctrlComment()->decisionComment($_GET);
                        } elseif ($_GET['action'] == 'delete') { // Permet de supprimer un chapitre ou un commentaire
                            ...
                            } elseif (isset($_GET['deleteTicket']) && (isset($_GET['idTicket'])) && ($_GET['deleteTicket'] == 'on')) { // Suppresion d'un chapitre
    ...
    public function setCtrlAdminSecure($ctrlAdminSecure)
    {
        $this->_ctrlAdminSecure = $ctrlAdminSecure;
    }
    // Getter ctrlAdminSecure
    public function ctrlAdminSecure()
    {
        return $this->_ctrlAdminSecure;
    }

    // Setter ctrlAdmin
    public function setCtrlAdmin($ctrlAdmin)
    {
        $this->_ctrlAdmin = $ctrlAdmin;
    }
    // Getter ctrlAdmin
    public function ctrlAdmin()
    {
        return $this->_ctrlAdmin;
    }

    // Setter ctrlTicket
    public function setCtrlTicket($ctrlTicket)
    {
        $this->_ctrlTicket = $ctrlTicket;
    }
    // Getter ctrlTicket
    public function ctrlTicket()
    {
        return $this->_ctrlTicket;
    }

    // Setter ctrlComment
    public function setCtrlComment($ctrlComment)
    {
        $this->_ctrlComment = $ctrlComment;
    }
    // Getter ctrlTicket
    public function ctrlComment()
    {
        return $this->_ctrlComment;
    }
}
